...
public class QuizActivity extends AppCompatActivity implements View.OnClickListener, ExoPlayer.EventListener {
    ...
    private void releasePlayer() {
        // TODO 3 - Call cancelAll on the notification manager when the activity is destroyed
    ...
    // TODO (1): Create a method that shows a MediaStyle notification with two actions
    // - play/pause
    // - skip to previous.
    //
    ...
    private void showNotification(PlaybackStateCompat stateCompat){
        // StateCompat is the current state of the Media session
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this);

        int icon;
        String play_pause;
        // Decides if we are going to use the play or pause action
        ...
        NotificationCompat.Action playPauseAction = new
                NotificationCompat.Action(icon, play_pause,
                // Ensures the appropriate media session callbacks are called automatically
                MediaButtonReceiver.buildMediaButtonPendingIntent(
                        this, PlaybackStateCompat.ACTION_PLAY_PAUSE)
        );

        // Notification action for restart
        NotificationCompat.Action restartAction = new
                android.support.v4.app.NotificationCompat.Action(
                        R.drawable.exo_controls_previous, getString(R.string.restart),
                // TODO 3.5 - Ensure the appropriate media session callbacks are called automatically
                MediaButtonReceiver.buildMediaButtonPendingIntent(
                        this, PlaybackStateCompat.ACTION_SKIP_TO_PREVIOUS));

        // Relaunches the Quiz activity when notification is clicked. Because the launch mode is
        // defined in the mainfest as single top the activity will be brought to the front instead
        // of being restarted, so we will keep our game progress.
        ...
        builder.setContentTitle(getString(R.string.guess))
                ...
                .setStyle(new NotificationCompat.MediaStyle()
                        ...
                        // Show both actions in the notification
    ...
    // TODO 4 - To receive the callback and action the buttons in the notification - See next exercise (TMED 06)
}
